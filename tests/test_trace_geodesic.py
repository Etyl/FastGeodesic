...
def test_common_edge_found():
    triangles = np.array([
        ...
        [2, 3, 4]   # Triangle 3 vertices
    ])
    
    # Common edge between triangle 0 and 1: vertices 1 and 2
    tri1 = 0
    tri2 = 1
    common, diff_verts = common_edge(triangles, tri1, tri2)
    
    # Expected common edge [1, 2] and remaining vertices
    expected_common = np.array([1, 2])
    expected_diff = ([0], [3])  # triangle 1 has vertex 0 left, triangle 2 has vertex 3 left
    
    assert np.array_equal(common, expected_common)
    assert diff_verts == expected_diff
...
def test_no_common_edge():
    ...
    # Expected result: no common edge [-1, -1]
    expected_common = np.array([-1, -1])
    expected_diff = []
...
def test_single_common_vertex():
    triangles = np.array([
        [0, 1, 2],  # Triangle 1 vertices
        [2, 1, 3],  # Triangle 2 vertices (shared vertex 1 and 2, but no full edge)
    ])
    ...
    assert np.array_equal(common, expected_common)
    assert diff_verts[0][0] == expected_diff[0][0]
    assert diff_verts[1][0] == expected_diff[1][0]
